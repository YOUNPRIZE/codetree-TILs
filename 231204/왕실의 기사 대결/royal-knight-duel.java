...
//            for (int j = 1; j <= L; j++) System.out.println(Arrays.toString(knightBoard[j]));
//            System.out.println();
        }
        int res = 0;
...
//        int hp = knight.hp;
        ...
        if (dir == 0) {
            ...
            loop: for (int i = knight.x - 1; i < knight.x + knight.height - 1; i++) {
                for (int j = knight.y; j < knight.y + knight.width; j++) {
                    if (board[i][j] == 1) attack++;
                    if (knight.hp - attack <= 0) {
                        flag = true;
            ...
            } else knights.set(idx-1, new Knight(idx, knight.x - 1, knight.y, knight.height, knight.width, knight.hp - attack, knight.damage + attack));
            // 오른쪽
        } else if (dir == 1) {
            for (int i = knight.x; i < knight.x + knight.height; i++) {
                knightBoard[i][knight.y] = 0;
            ...
            if (flag) {
                ...
                knights.set(idx-1, new Knight(idx, 0, 0, 0, 0, 0, 0));
            } else knights.set(idx-1, new Knight(idx, knight.x, knight.y + 1, knight.height, knight.width, knight.hp - attack, knight.damage + attack));
            // 아래
        } else if (dir == 2) {
            for (int j = knight.y; j < knight.y + knight.width; j++) {
                knightBoard[knight.x][j] = 0;
